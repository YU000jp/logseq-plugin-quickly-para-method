...
export const openPARAfromToolbar = async () => {
  ...
  <div title="" style="user-select: none">
    <ul>
      ...
      <hr/>
      <li class="para-away"><span>/✈️ [Projects]</span><span>${paraBoolean ? `<small><button title="${t("Tag the current page (tags property)")}" data-on-click="Projects">${t("Tag")}</button></small> | ` : ''}<small><button id="paraOpenButtonProjects" title="${t("Press Shift key at the same time to open in sidebar")}">${t("Open")}</button></small></span></li>
      <li class="para-away"><span>/🏠 [Areas of responsibility]</span><span>${paraBoolean ? `<small><button title="${t("Tag the current page (tags property)")}" data-on-click="AreasOfResponsibility">${t("Tag")}</button></small> | ` : ''}<small><button id="paraOpenButtonAreas" title="${t("Press Shift key at the same time to open in sidebar")}">${t("Open")}</button></small></span></li>
      <li class="para-away"><span>/🌍 [Resources]</span><span>${paraBoolean ? `<small><button title="${t("Tag the current page (tags property)")}" data-on-click="Resources">${t("Tag")}</button></small> | ` : ''}<small><button id="paraOpenButtonResources" title="${t("Press Shift key at the same time to open in sidebar")}">${t("Open")}</button></small></span></li>
      <li class="para-away"><span>/🧹 [Archives]</span><span>${paraBoolean ? `<small><button title="${t("Tag the current page (tags property)")}" data-on-click="Archives">${t("Tag")}</button></small> | ` : ''}<small><button id="paraOpenButtonArchives" title="${t("Press Shift key at the same time to open in sidebar")}">${t("Open")}</button></small></span></li>
    </ul>
    <hr/>
      `
  } else {
